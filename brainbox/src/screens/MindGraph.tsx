...
import { 
  ...
  Maximize2, Minimize2, Filter, Info
} from 'lucide-react';
  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden bg-transparent font-sans">
      ...
      <div className="flex-1 flex flex-col lg:flex-row z-10 relative overflow-hidden">
        ...
        {/* Left: Neural Graph Field (Full Space) */}
        <div className={`relative flex-1 flex flex-col transition-all duration-500 ${isFullScreen ? 'lg:flex-[10]' : 'lg:flex-[2.5]'}`} ref={containerRef}>
          ...
          {/* Node Details Overlay (Bottom Left) */}
          <AnimatePresence>
            {selectedNode && (
              <motion.div
        ...
        {/* Right: Intelligence Stats (Sidebar) */}
        <AnimatePresence>
          {!isFullScreen && (
            <motion.div 
            ...
            >
              ...
              <div className="glass-panel p-6 rounded-3xl border border-white/5">
                ...
                <div className="space-y-3">
                  {[
                    { icon: Database, label: 'Knowledge Growth', val: '+124', unit: 'Artifacts', color: 'emerald' },
                    { icon: Clock, label: 'Time Optimized', val: '14.5h', unit: 'Saved', color: 'blue' },
                    { icon: Zap, label: 'Neural Sync', val: '94%', unit: 'Accuracy', color: 'purple' }
                  ].map((item, i) => (
                    <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all group cursor-default">
                      <div className={`w-10 h-10 rounded-xl bg-${item.color}-500/10 flex items-center justify-center group-hover:scale-110 transition-transform`}>
                      ...
                      <div className={`ml-auto text-${item.color}-500 text-xs font-mono font-bold`}>{item.val}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* The Time Slider (Timeline Explorer) */}
