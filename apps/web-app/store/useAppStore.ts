...
interface AppState {
  ...
  // Modal Data
  targetModel: Record<string, unknown> | null;
...
interface AppActions {
  ...
  setSwitchMode: (
    mode: 'global' | 'folders' | 'feathers' | 'pulse' | 'workspace',
    direction?: number
  ) => void;
  ...
  setModalOpen: (
    modal: 'newFolder' | 'newChat' | 'smartSwitch' | 'apiKey',
    open: boolean,
  ...
  persist(
    (set, get) => ({
      ...
      setModalOpen: (modal, open, data) => {
        switch (modal) {
          case 'newFolder':
            set({ isNewFolderModalOpen: open });
            break;
          case 'newChat':
            set({ isNewChatModalOpen: open });
            break;
          case 'smartSwitch':
            set({ isSmartSwitchModalOpen: open, targetModel: data });
            break;
          case 'apiKey': {
            const d = data as { id?: string; name?: string } | undefined;
            set({ isApiKeyModalOpen: open, apiKeyModelId: d?.id ?? null, apiKeyModelName: d?.name ?? null });
            break;
          }
        }
      },
