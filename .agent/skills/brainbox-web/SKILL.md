...
Target: **pnpm monorepo** → `apps/web-app` (Next.js 16.2) + `apps/extension` (Vite 8, future)
...
│   └── extension/        ← Vite 8 (future, scaffold only)
├── packages/
...
// proxy.ts — Supabase session refresh (apps/web-app/proxy.ts, NOT middleware.ts)
...
- `NeuralField` must be `'use client'` in `packages/ui`
- The canvas is `position: fixed` — it persists across ALL screen changes
- The `mode` prop changes drive the particle transition (brain↔wander↔extension)
- Mode is derived from `activeScreen` in the store — NOT from routing
...
```typescript
// proxy.ts (at apps/web-app root) — NOT middleware.ts in Next.js 16.2
// Protects all routes except /
// Uses @supabase/ssr for cookie-based sessions
// Function name: export async function proxy(request: NextRequest)
...
- [ ] `apps/extension` scaffolded (empty Vite 8 app, ready for future work)
