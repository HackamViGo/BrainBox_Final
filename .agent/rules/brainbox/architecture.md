...
1. **Extension → HTTPS Bearer JWT → /api/** → Supabase
2. **web-app API routes → @supabase/ssr** → PostgreSQL (RLS)
3. **/extension-auth → postMessage** → authManager → chrome.storage.local

## Package Rules

- `@brainbox/types`: Single source of truth for ALL types.
- `@brainbox/ui`: shadcn/ui components + design tokens.
- `@brainbox/utils`: Logger, crypto (AES-GCM), constants.
- `@brainbox/config`: tsconfig.base, eslint, CSS tokens.
- **New shared dependency** → add to pnpm catalog first.
...
| `docs/AGENTS_GRAPH.json` | .agent/ папката (skills, rules, workflows) | Само агент (ръчно) |
